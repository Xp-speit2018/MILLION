...
GPT2_LAYER = transformers.models.gpt2.modeling_gpt2.GPT2Block
MPT_MODEL = transformers.models.mpt.modeling_mpt.MptForCausalLM
MPT_LAYER = transformers.models.mpt.modeling_mpt.MptBlock
QWEN2MOE_MODEL = transformers.models.qwen2_moe.modeling_qwen2_moe.Qwen2MoeForCausalLM
QWEN2MOE_LAYER = transformers.models.qwen2_moe.modeling_qwen2_moe.Qwen2MoeDecoderLayer
...
def get_layers(model):
    ...
    if isinstance(model, GPT2_MODEL):
        return model.transformer.h
    if isinstance(model, MPT_MODEL):
        return model.transformer.blocks
    if isinstance(model, QWEN2MOE_MODEL):
        return model.model.layers
    raise NotImplementedError
...
def get_embeddings(model, model_type) -> list[torch.nn.Module]:
    ...
    elif model_type == GPT2_MODEL:
        return [model.transformer.wte, model.transformer.wpe]
    elif model_type == MPT_MODEL:
        return [model.transformer.wte, model.transformer.wpe]
    elif model_type == QWEN2MOE_MODEL:
        return [model.model.embed_tokens, model.model.embed_positions]
    else:
...
def get_transformer_layers(model, model_type):
    if model_type == LLAMA_MODEL:
        ...
        return [layer for layer in model.transformer.h]
    elif model_type == MPT_MODEL:
        return [layer for layer in model.transformer.blocks]
    elif model_type == QWEN2MOE_MODEL:
        return [layer for layer in model.model.layers]
    else:
        raise ValueError(f'Unknown model type {model_type}')
